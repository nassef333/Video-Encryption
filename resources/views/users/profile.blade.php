...
<div class="nav-align-top mb-4">
                    ...
                    <div class="tab-content">
                      <div class="tab-pane fade show active" id="navs-pills-top-profile" role="tabpanel">
                        <div class="card mb-4">
                            <h5 class="card-header"> User ID: {{ $user->id }}
                            </h5>
                            <!-- Account -->
                            <div class="card-body">
                            ...
                            <div class="card-body">
                                <form action="/user/{{$user->id}}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3">
                                      <label class="form-label" for="basic-default-fullname">Parent Phone</label>
                                      <input type="text" class="form-control" id="basic-default-fullname" placeholder="Parent Phone" name="pphone" value="{{$user->pphone}}" required>
                                      @error('pphone')
                                      <div class="text-danger">{{ $message }}</div>
                                      @enderror
                                    </div>   
                                    <div class="mb-3">
                                      ...
                                      <input type="text" class="form-control" id="basic-default-fullname" placeholder="email" name="phone" value="{{$user->phone}}" required>
                                      @error('phone')
                                      <div class="text-danger">{{ $message }}</div>
                                      @enderror
                                    </div>   
                                    <div class="mb-3">
                                      <label class="form-label" for="basic-default-fullname">Government</label>
                                      <input type="text" class="form-control" id="basic-default-fullname" placeholder="government" name="government" value="{{$user->government}}" required>
                                      @error('government')
                                      <div class="text-danger">{{ $message }}</div>
                                      @enderror
                                    </div>   
                                    <div class="mb-3">
                                      ...
                                      <input type="text" class="form-control" id="basic-default-fullname" placeholder="level" name="level" value="{{$user->level}}" required>
                                      @error('level')
                                      <div class="text-danger">{{ $message }}</div>
                                      @enderror
                                    </div>  
                        ...
                        <table class="table">
                            ...
                            <tbody class="table-border-bottom-0">
                              @foreach ($courses as $course)
                                <tr id="deleteRow{{$course->id}}">
                                  <td>{{ $course->name }}</td>
                                  <td>
                                    @if ($course->img)
                                    <div class="image-overlay">
                                      <img src="{{ asset("storage/".$course->img)}}" alt="{{ $course->name }}" width="100" class="overlay-image" onclick="toggleOverlay(this)">
                                    </div>
                                    @endif
                                  </td>              <td>{{ $course->price }}</td>
                                  <td>{{ $course->created_at }}</td>
                                  <td>
                                    <input
                                      data-id="{{ $course->is_published }}"
                                      class="toggle-class-publish form-check-input"
                                      type="checkbox"
                                      data-toggle="toggle"
                                  ...
                                  <td class="text-center">
                                    <div class="btn-group">
                                      <form action="/courses/{{$course->id}}" method="POST">
                                        @method('DELETE')
                                        @csrf
                                        <a href="/courses/{{$course->id}}/content" type="button" class="btn btn-outline-secondary btn-sm"><i class="fa-solid fa-arrows-to-circle"></i> </a>
                                        <a href="/courses/{{$course->id}}/students" type="button" class="btn btn-outline-success btn-sm"><i class="fa-solid fa-people-group"></i></a>
                                        <a href="/courses/{{$course->id}}/edit" type="button" class="btn btn-outline-primary btn-sm"><i class="fa-regular fa-pen-to-square"></i></a>
                                        <button type="button" onclick="showConfirmationModal({{$course->id}})" class="btn btn-outline-danger btn-sm"><i class="fa-regular fa-trash-can"></i></button>
                                      </form>
                                  </div>
                                  </td>
                                </tr>
