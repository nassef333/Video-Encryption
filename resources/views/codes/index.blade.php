...
<div class="card">
  ...
  <div class="card-body">
        ...
        <tbody class="table-border-bottom-0">
          @foreach ($codes as $code)
            <tr id="deleteRow{{$code->id}}">
              ...
              <td>
                @if ($user_week = $code->userWeeks->first())
                <button class="btn btn-outline-primary btn-sm">
                  @php
                    $user_id = $code->userWeeks->first()->user_id;
                    $data = App\Models\Users::find($user_id)->first();
              ...
              <td class="text-center">
                <div class="btn-group">
                  <form action="/courses/{{$code->id}}" method="POST">
                    @method('DELETE')
                    @csrf
                    <a href="/clear-code/{{ $code->id }}" type="button" class="btn btn-outline-warning btn-sm">clear</a>
                    <button type="button" onclick="showConfirmationModal({{$code->id}})" class="btn btn-outline-danger btn-sm">Delete</button>
                  </form>
              </div>
              </td>
          @endforeach
      ...
      <nav aria-label="Page navigation">
        <ul class="pagination">
          ...
          <li class="page-item {{ $codes->onFirstPage() ? 'disabled' : '' }}">
            <a class="page-link" href="{{ $codes->url(1) }}" aria-label="First">
          ...
          <!-- Previous Page Link -->
          <li class="page-item {{ $codes->onFirstPage() ? 'disabled' : '' }}">
            <a class="page-link" href="{{ $codes->previousPageUrl() }}" aria-label="Previous">
          ...
          @for ($page = $startPage; $page <= $endPage; $page++)
            <li class="page-item {{ $page == $codes->currentPage() ? 'active' : '' }}">
              <a class="page-link" href="{{ $codes->url($page) }}">{{ $page }}</a>
            </li>
          @endfor

          <!-- Next Page Link -->
          <li class="page-item {{ $codes->currentPage() == $codes->lastPage() ? 'disabled' : '' }}">
            <a class="page-link" href="{{ $codes->nextPageUrl() }}" aria-label="Next">
          ...
          <!-- Last Page Link -->
          <li class="page-item {{ $codes->currentPage() == $codes->lastPage() ? 'disabled' : '' }}">
            <a class="page-link" href="{{ $codes->url($codes->lastPage()) }}" aria-label="Last">
    ...
    <div class="d-flex justify-content-center">
      <p>Page {{ $codes->currentPage() }} of {{ $codes->lastPage() }}</p>
    </div>
