...
<div class="card">
  <h2 class="card-header text-center">Questions</h2>
  <p class="text-center">Total Questions: {{ $questions->total() }}</p>
  <div class="card-body">
    <div class="search mt-2 mb-5">
        <div class="buttons mb-2">
          <a href="/quiz/{{ request()->segment(2) }}/question/create" type="button" class="btn btn-outline-primary btn-sm">+ Add Question</a>
        </div>
      <div class="buttons mb-2">
    ...
    <form action="/exams/{{ request()->segment(2) }}/questions" method="GET" class="form-inline">
      @csrf
      ...
      <table class="table">
        ...
        <tbody class="table-border-bottom-0">
          @foreach ($questions as $question)
            <tr id="deleteRow{{$question->id}}">
              <td>
                @if ($question->img)
                <div class="image-overlay">
                  <img src="{{ asset("storage/".$question->img)}}" alt="{{ $question->question }}" width="100" class="overlay-image" onclick="toggleOverlay(this)">
                </div>
                @endif
              </td>
              ...
              <td class="text-center">
                <div class="btn-group">
                  <form action="/questions/{{$question->id}}" method="POST">
                    @method('DELETE')
                    @csrf
                    <a href="/questions/{{$question->id}}/edit" type="button" class="btn btn-outline-primary btn-sm">Edit</a> <!-- Added btn-sm class -->
                    <button type="button" onclick="showConfirmationModal({{$question->id}})" class="btn btn-outline-danger btn-sm">Delete</button>
                  </form>
              </div>
              </td>
            </tr>
      ...
      <nav aria-label="Page navigation">
        <ul class="pagination">
          ...
          <li class="page-item {{ $questions->onFirstPage() ? 'disabled' : '' }}">
            <a class="page-link" href="{{ $questions->url(1) }}" aria-label="First">
          ...
          @for ($page = $startPage; $page <= $endPage; $page++)
            <li class="page-item {{ $page == $questions->currentPage() ? 'active' : '' }}">
              <a class="page-link" href="{{ $questions->url($page) }}">{{ $page }}</a>
            </li>
          @endfor
          ...
          <!-- Last Page Link -->
          <li class="page-item {{ $questions->currentPage() == $questions->lastPage() ? 'disabled' : '' }}">
            <a class="page-link" href="{{ $questions->url($questions->lastPage()) }}" aria-label="Last">
    ...
    <div class="d-flex justify-content-center">
      <p>Page {{ $questions->currentPage() }} of {{ $questions->lastPage() }}</p>
    </div>
