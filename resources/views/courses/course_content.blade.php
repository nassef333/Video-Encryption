        ...
        <div class="layout-page">
          ...
          <div class="content-wrapper">
            ...
            <div class="container-fluid flex-grow-1 container-p-y">
                ...
                <a class="btn btn-primary btn-buy-now" href="/course/{{ $course->id }}/week/create">Add Week</a>
                <a class="btn btn-dark btn-buy-now" href="/add-exist-week/{{ $course->id }}">Add Exist Week</a>
                <div class="accordion mt-3 accordion-without-arrow" id="accordionExample">
                    @foreach ($weeks as $week)
                    <div class="accordion-item card" data-week-id="{{ $week->id }}">
                      <a href="/weeks/{{ $week->id }}/content">
                        <h2 class="accordion-header text-body d-flex justify-content-between" id="headingThree">
                          ...
                          <form action="/weeks/{{$week->id}}" method="POST">
                            @method('DELETE')
                            @csrf
                            <a href="/weeks/{{ $week->id }}/content" type="button" class="btn btn-outline-primary btn-sm"><i class="fa-regular fa-folder-open"></i></a>
                            <a href="/week/{{ $week->id }}/sort" type="button" class="btn btn-outline-dark btn-sm"><i class="fa-solid fa-arrow-down-1-9"></i></a>
                            <button type="button" onclick="showConfirmationModal({{$week->id}})" class="btn btn-outline-danger btn-sm">
...
<div class="modal fade" id="confirmationModal" tabindex="-1" role="dialog" aria-labelledby="confirmationModalLabel" aria-hidden="true">
    ...
    function deleteProduct(ProductId) {
        $.ajax({
            url: '/course/{{ request()->segment(2) }}/week/' + ProductId + '/clear/',
            type: 'GET',
            data: {
                _token: '{{ csrf_token() }}'
            },
            success: function() {
                // Remove the row from the accordion
                $('#accordionExample .accordion-item[data-week-id="' + ProductId + '"]').remove();
            },
            error: function() {
