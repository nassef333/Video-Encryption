...
<div class="card">
  <h2 class="card-header text-center">Videos</h2>
  <p class="text-center">Total Videos: {{ $videos->total() }}</p>
  <div class="card-body">
    <div class="search mt-2 mb-5">
        <div class="buttons mb-2">
          <a href="/videos/create" type="button" class="btn btn-outline-primary btn-sm">+ Add Video</a>
        </div>
      <div class="buttons mb-2">
              ...
              <td class="text-center">
                <div class="btn-group">
                  <form action="/videos/{{$video->id}}" method="POST">
                    @method('DELETE')
                    @csrf
                    <a href="/videos/{{$video->id}}/edit" type="button" class="btn btn-outline-primary btn-sm">Edit</a> <!-- Added btn-sm class -->
                    <button type="button" onclick="showConfirmationModal({{$video->id}})" class="btn btn-outline-danger btn-sm">Delete</button>
                  </form>
              </div>
              </td>
            </tr>
      ...
      <nav aria-label="Page navigation">
        <ul class="pagination">
          ...
          <li class="page-item {{ $videos->onFirstPage() ? 'disabled' : '' }}">
            <a class="page-link" href="{{ $videos->url(1) }}" aria-label="First">
          ...
          @for ($page = $startPage; $page <= $endPage; $page++)
            <li class="page-item {{ $page == $videos->currentPage() ? 'active' : '' }}">
              <a class="page-link" href="{{ $videos->url($page) }}">{{ $page }}</a>
            </li>
          @endfor
          ...
          <!-- Last Page Link -->
          <li class="page-item {{ $videos->currentPage() == $videos->lastPage() ? 'disabled' : '' }}">
            <a class="page-link" href="{{ $videos->url($videos->lastPage()) }}" aria-label="Last">
    ...
    <div class="d-flex justify-content-center">
      <p>Page {{ $videos->currentPage() }} of {{ $videos->lastPage() }}</p>
    </div>
