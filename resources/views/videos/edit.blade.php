              ...
              <div class="col-xl">
                <div class="card mb-4">
                  <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="mb-4">Edit {{$video->name}}</h4>
                    <small class="text-muted float-end">Edit video</small>
                  </div>
                  <div class="card-body">
                    <form action="/videos/{{$video->id}}" method="POST" enctype="multipart/form-data">
                      ...
                      <input type="hidden" name="_method" value="PUT">
                        <div class="mb-3">
                          ...
                          <input type="text" class="form-control" id="basic-default-fullname" placeholder="video title" name="title" value="{{ $video->title }}">
                          @error('title')
                        ...
                        <div class="mb-3">
                          <label class="form-label" for="basic-default-fullname">iframe</label>
                          <input type="text" class="form-control" id="basic-default-fullname" placeholder="iframe" name="iframe" value="{{ $video->iframe }}">
                          @error('iframe')
                          <div class="text-danger">{{ $message }}</div>
                          @enderror
                        </div> 
                        ...
                        <div class="d-flex">
                          <div class="mb-3 w-50">
                            <label class="form-label" for="basic-default-fullname">number of views</label>
                            <input type="text" class="form-control" id="basic-default-fullname" placeholder="number of views" name="noviews" value="{{ $video->noviews }}">
                            @error('noviews')
                            <div class="text-danger">{{ $message }}</div>
                            @enderror
                          </div>  
                          <div class="mb-3 w-50">
                            <label class="form-label" for="basic-default-fullname">Time Between Views</label>
                            <input type="text" class="form-control" id="basic-default-fullname" placeholder="Time Between Views" name="minutes_views" value="{{ $video->minutes_views }}">
                            @error('minutes_views')
                            <div class="text-danger">{{ $message }}</div>
                            @enderror
                          </div>      
                        ...
                        <div class="d-flex">                   
                          <div class="mb-3 w-50">
                            ...
                            <select class="form-control" id="basic-default-fullname" name="type">
                              <option value="local" {{ $video->type == "local" ? 'selected' : '' }}>local</option>
                              <option value="youtube" {{ $video->type == "youtube" ? 'selected' : '' }}>youtube</option>
                              <option value="vimeo" {{ $video->type == "vimeo" ? 'selected' : '' }}>vimeo</option>
                              <option value="videocipher" {{ $video->type == "videocipher" ? 'selected' : '' }}>videocipher</option>
                              <option value="bunny" {{ $video->antypeswer == "bunny" ? 'selected' : '' }}>bunny</option>
                            </select>
                            @error('type')
                            <div class="text-danger">{{ $message }}</div>
                            @enderror
                          ...
                          <div class="mb-3 w-50">
                            <label class="form-label" for="basic-default-fullname">Video Dauration</label>
                            <input type="text" class="form-control" id="basic-default-fullname" placeholder="Time Between Views" name="video_dauration" value="{{ $video->video_dauration }}">
                            @error('video_dauration')
                            <div class="text-danger">{{ $message }}</div>
                            @enderror
                          </div>   
                        </div>
                        ...
                        <div class="mb-3 w-50">
                          ...
                          <select class="form-control" id="basic-default-fullname" name="level">
                            <option value="1" {{ $video->level == 1 ? 'selected' : '' }}>الأول الثانوي</option>
                            <option value="2" {{ $video->level == 2 ? 'selected' : '' }}>الثاني الثانوي</option>
                          </select>                          
                          @error('level')
                          <div class="text-danger">{{ $message }}</div>
                          @enderror
                        ...
                        <div class="mb-3 w-50">
                          <label class="form-label" for="basic-default-fullname">week</label>
                          <select class="form-control" id="basic-default-fullname" name="week_id">
                            @foreach ($weeks as $week)
                              <option value="{{ $week->id }}" {{ $video->wek_id ==  $week->id  ? 'selected' : '' }}>{{ $week->name }}</option>
                            @endforeach
                          </select>
                          @error('week_id')
                          <div class="text-danger">{{ $message }}</div>
