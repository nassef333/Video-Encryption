...
<div class="card">
  <h2 class="card-header text-center">Transactions</h2>
  <p class="text-center">Total Transactions: {{ $transactions->total() }}</p>
  <div class="card-body">
    ...
    <form action="{{ route('transactions.index') }}" method="GET" class="form-inline">
      @csrf
        <div class="input-group">
            ...
            <input type="text" class="form-control" placeholder="Search with payment id or invoice id or reference id" aria-label="Search" aria-describedby="basic-addon-search31" name="searchTerm" value="{{ request('searchTerm') }}">
      ...
      <table class="table">
        ...
        <tbody class="table-border-bottom-0">
          @foreach ($transactions as $transaction)
            <tr id="deleteRow{{$transaction->id}}">

              <td>{{ $transaction->id }}</td>
              <td>{{ $transaction->userWeeks[0]['id'] }}</td>
              <td>
                @php
                  $username = App\Models\Users::find($transaction->userWeeks[0]['id'])->first();
                  echo $username->fname . ' ' . $username->lname 
                @endphp  
              </td>
              <td>{{ $transaction->id }}</td>
              ...
              <td>
                @php
                  $coursename = App\Models\Course::find($transaction->userWeeks[0]['week_id'])->first();
      ...
      <nav aria-label="Page navigation">
        <ul class="pagination">
          ...
          <li class="page-item {{ $transactions->onFirstPage() ? 'disabled' : '' }}">
            <a class="page-link" href="{{ $transactions->url(1) }}" aria-label="First">
          ...
          <!-- Last Page Link -->
          <li class="page-item {{ $transactions->currentPage() == $transactions->lastPage() ? 'disabled' : '' }}">
            <a class="page-link" href="{{ $transactions->url($transactions->lastPage()) }}" aria-label="Last">
    ...
    <div class="d-flex justify-content-center">
      <p>Page {{ $transactions->currentPage() }} of {{ $transactions->lastPage() }}</p>
    </div>
